...
class Keyring(CredsPlugin):
    ...
    def buildserver_credentials(self):
        ...
        """
        return {
            'buildserver-user': self.keyring.get_password(
                self.section, 'buildserver-user'
            ),
            'buildserver-token': self.keyring.get_password(
                self.section, 'buildserver-token'
            )
        }
    ...
    def chef_credentials(self):
        ...
        """
        return {
            'chef_server': self.keyring.get_password(
                self.section, 'chef-server'
            ),
            'chef_username': self.keyring.get_password(
                self.section, 'chef-username'
            ),
            'chef_keyfile': self.keyring.get_password(
                self.section, 'chef-keyfile'
            ),
            'chef_client_user': self.keyring.get_password(
                self.section, 'chef-client-user'
            ),
            'chef_client_keyfile': self.keyring.get_password(
                self.section, 'chef-client-keyfile'
            )
        }
    ...
    def set_chef_credentials(
        ...
        self.keyring.set_password(
            self.section, 'chef-client-user', client_user
        )
        self.keyring.set_password(
            self.section, 'chef-client-keyfile', client_key
        )

    def dockerhub_credentials(self):
        return {
            'username': self.keyring.get_password(
                self.section, 'docker_login_username'
            ),
            'email': self.keyring.get_password(
                self.section, 'docker_login_email'
            ),
            'password': self.keyring.get_password(
                self.section, 'docker_login_password'
            )
        }
    ...
    def set_dockerhub_credentials(self, email, user, password):
        ...
        self.keyring.set_password(
            self.section, 'docker_login_email', email
        )
        self.keyring.set_password(
            self.section, 'docker_login_password', password
        )

    def file_server_credentials(self):
        return {
            'file_server_username': self.keyring.get_password(
                self.section, 'file-server-username'
            ),
            'file_server_keyfile': self.keyring.get_password(
                self.section, 'file-server-keyfile'
            )
        }

    def set_file_server_credentials(self, username, keyfile):
        self.keyring.set_password(
            self.section, 'file-server-username', username
        )
        self.keyring.set_password(
            self.section, 'file-server-keyfile', keyfile
        )
